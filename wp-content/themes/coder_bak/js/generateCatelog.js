 ...
 */
 ...
 function generateLevel (target, maxLevel, current) {
	 ...
	 for (var i = 0; i < children.length; i++) {
		 ...
		 var next = nextLevel(child, util, maxLevel, current + 1, $(allChildren[allChildren.length - 1]));
		 result += next;
	 }
	 result += '</ul></li>';
	 return result;
 ...
 function nextLevel(target, util, maxLevel, current, lastEle) {
	 ...
	 for (var i = 0; i < nextEles.length; i++) {
		 ...
		 var text = next.text();
		 var name = "category_" + current + "_" + i;
		 result += '<li><a href="#' + name + '">' + text + '</a></li>';
		 var ha = "<a name='" + name + "'>" + text +  "</a>";
		 next.html(ha);
		 var nextUtil;
		 ...
		 result += nextLevel(next, nextUtil, maxLevel, current + 1, lastEle);
	 }
	 result += '</ul>';
	 return result;
 }
